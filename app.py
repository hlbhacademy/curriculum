...
# Google Sheets 認證（從環境變數抓 JSON 字串）
...
scheduler.add_job(update_cache, trigger='interval', minutes=3)
scheduler.start()
...
def login():
    ...
    session["nonce"] = nonce
    return google.authorize_redirect(redirect_uri=url_for("callback", _external=True), nonce=nonce)
